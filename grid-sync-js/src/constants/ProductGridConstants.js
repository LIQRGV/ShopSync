 ...
 */
function getProductStatusText(numericValue) {
    return PRODUCT_STATUS.NUMERIC_TO_TEXT[numericValue] || 'Unknown';
}

/**
 * Get product status numeric value by text
 */
function getProductStatusNumeric(textValue) {
    return PRODUCT_STATUS.TEXT_TO_NUMERIC[textValue] || 1;
}

/**
 * Get product status CSS class by numeric value
 */
function getProductStatusClass(numericValue) {
    return PRODUCT_STATUS.CSS_CLASSES[numericValue] || 'status-default';
}

/**
 * Get sell status text by numeric value
 */
function getSellStatusText(numericValue) {
    // Handle null, undefined, or 0 as "Sell as Standard" (default)
    if (numericValue === null || numericValue === undefined || numericValue === 0) {
        return 'Sell as Standard';
    }
    return SELL_STATUS.NUMERIC_TO_TEXT[numericValue] || 'Unknown';
}

/**
 * Get sell status numeric value by text
 */
function getSellStatusNumeric(textValue) {
    return SELL_STATUS.TEXT_TO_NUMERIC[textValue] || 1;
}

/**
 * Get VAT scheme text by numeric value
 */
function getVatSchemeText(numericValue) {
    return VAT_SCHEME.NUMERIC_TO_TEXT[numericValue] || 'Unknown';
}

/**
 * Get VAT scheme numeric value by text
 */
function getVatSchemeNumeric(textValue) {
    return VAT_SCHEME.TEXT_TO_NUMERIC[textValue] || 0;
}

// Export as object with all constants and functions
