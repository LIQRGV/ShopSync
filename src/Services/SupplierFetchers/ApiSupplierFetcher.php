...
class ApiSupplierFetcher implements SupplierFetcherInterface
{
    ...
    /**
     * Get all suppliers from client API
     *
     * @param array $filters
    ...
    public function getAll(array $filters = [], array $includes = [])
    {
        ...
        $response = $this->handleRequest(function () use ($params) {
            return $this->httpClient()->get('/suppliers', $params);
        }, ['data' => []]);
    ...
    /**
     * Paginate suppliers from client API
     *
     * @param int $perPage
     * @param array $filters
     * @param array $includes
    ...
    /**
     * Find supplier by ID from client API
     *
     * @param mixed $id
     * @return array|null
     */
    public function find($id)
    {
        $response = $this->handleRequest(function () use ($id) {
            return $this->httpClient()->get("/suppliers/{$id}");
        }, null);
    ...
    /**
     * Create supplier via client API
     *
     * @param array $data
     * @return array
     */
    public function create(array $data)
    {
        $response = $this->handleRequest(function () use ($data) {
            return $this->httpClient()->post('/suppliers', $data);
        }, ['data' => []]);
    ...
    /**
     * Update supplier via client API
     *
     * @param mixed $id
     * @param array $data
     * @return array|null
    ...
    public function update($id, array $data)
    {
        $response = $this->handleRequest(function () use ($id, $data) {
            return $this->httpClient()->put("/suppliers/{$id}", $data);
        }, null);
    ...
    /**
     * Delete supplier via client API
     *
     * @param mixed $id
     * @return bool
     */
    ...
    {
        $response = $this->handleRequest(function () use ($id) {
            return $this->httpClient()->delete("/suppliers/{$id}");
        }, null);
    ...
    protected function handleRequest(callable $callback, $default = null)
    {
        try {
            ...
            \Log::error('API Supplier Fetcher request failed', [
            ...
            \Log::error('API Supplier Fetcher exception', [
                'error' => $e->getMessage(),
                'client_id' => $this->client->id
            ]);
