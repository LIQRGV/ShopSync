...
class ApiBrandFetcher implements BrandFetcherInterface
{
    ...
    /**
     * Get all brands from client API
     *
     * @param array $filters
    ...
    /**
     * Paginate brands from client API
     *
     * @param int $perPage
     * @param array $filters
     * @param array $includes
    ...
    /**
     * Find brand by ID from client API
     *
     * @param mixed $id
     * @return array|null
     */
    public function find($id)
    {
        $response = $this->handleRequest(function () use ($id) {
            return $this->httpClient()->get("/brands/{$id}");
        }, null);
    ...
    public function create(array $data)
    {
        $response = $this->handleRequest(function () use ($data) {
            return $this->httpClient()->post('/brands', $data);
        }, ['data' => []]);
    ...
    public function update($id, array $data)
    {
        $response = $this->handleRequest(function () use ($id, $data) {
            return $this->httpClient()->put("/brands/{$id}", $data);
        }, null);
    ...
    {
        $response = $this->handleRequest(function () use ($id) {
            return $this->httpClient()->delete("/brands/{$id}");
        }, null);
    ...
    protected function handleRequest(callable $callback, $default = null)
    {
        try {
            ...
            \Log::error('API Brand Fetcher request failed', [
            ...
            \Log::error('API Brand Fetcher exception', [
                'error' => $e->getMessage(),
                'client_id' => $this->client->id
            ]);
